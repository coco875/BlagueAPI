...
type_joke = ["global", "dev", "dark","limit", "beauf", "blondes"]
...
class Joke:
    def __init__(self, token, ressemblance=0.75, *args):
        """mettre son token obtenue sur https://www.blagues-api.fr/ et ressemblance est pour changer le pourcentage de ressemblance minimum pour considérer un type appelé avec Joke.get_joke_type(type) comme valide, rajouter a la fin les types que vous voulez pas que ça sorte quand il y a pas de type indiqué (si il reconnait pas ou que le taux de ressemblence et inférieur a celui indiqué alors il passe)"""
        self.token = token
        self.warn = True
        ...
        if len(args)>0:
            ...
            for i in range(len(args)):
                type_detect = comparaison_proche(args[i], type_joke, self.ressemblance)
    ...
    def get_joke(self):
        ...
        return rep.json()
    def get_joke_type(self,genre=None):
        """permet d'obtenir une blague d'un certain genre même si il y a des fautes dans le genre. c'est considérer comme valide si le mot d'après la Distance de Jaro-Winkler (un truc pour savoir la ressemblence) est ressemblant a 75%"""
        if type(genre) is not str:
            rep = requests.get(f"https://www.blagues-api.fr/api/random{self.disallow}", headers = {'Authorization': f'Bearer {self.token}'})
            return rep.json()
        else:
            type_detect = comparaison_proche(genre, type_joke, self.ressemblance)
            if len(type_detect)>0:
                rep = requests.get(f"https://www.blagues-api.fr/api/type/{type_detect}/random", headers = {'Authorization': f'Bearer {self.token}'})
                return rep.json()
            else:
                if self.warn:
                    print("Warning: rentré un nom plus rensemblent a un genre existant (vous pouvez savoir la liste a partir de ça BlagueApiFr.type_joke)")
                    self.warn = False
                return {"id" : -1, "error": "type non rensemblant a ceux existant"}
    def get_joke_type_strict(self, genre=None):
    ...
    def get_joke_exclu_type(self,*args):
        ...
        if len(args)>0:
            ...
            return rep.json()
    def get_joke_exclu_type_strict(self,*args):
